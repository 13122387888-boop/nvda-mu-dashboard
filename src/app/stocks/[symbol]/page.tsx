...
export default async function StockPage({ params, searchParams }: { params: Promise<{ symbol: string }>; searchParams: Promise<{ window?: string | string[] }> }) {
  ...
  return (
    <main className="shell">
      ...
      <section className="section-block" id="module-price"><ModuleHeading index="01" kicker="PRICE STRUCTURE" title="价格趋势与关键位" description="价格处在什么趋势，离重要期权价位还有多远。" canAnswer="趋势位置与关键价位距离" cannotAnswer="突破后的必然涨跌方向" accent="#57d68d" />
      ...
      <section className="section-block" id="module-momentum"><ModuleHeading index="02" kicker="MOMENTUM & VOLATILITY" title="动量与波动率" description="判断走势是否过热，并比较近期实际波动与期权隐含定价。" canAnswer="动量冷热与波动定价差异" cannotAnswer="下一交易日涨跌或期权绝对贵贱" accent="#4f8cff" />
      ...
      <section className="section-block" id="module-options"><ModuleHeading index="03" kicker="OPTIONS POSITIONING" title="期权持仓结构" description="观察未平仓量集中位置、预期区间与 Gamma 结构代理。" canAnswer="OI集中位置与到期波动定价" cannotAnswer="真实做市商方向或未来价格" accent="#f0b45c" />
