...
import { HistoricalPosition, OptionWindowSelector, SnapshotLink } from "@/components/product-insights";
...
export default async function StockPage({ params, searchParams }: { params: Promise<{ symbol: string }>; searchParams: Promise<{ window?: string | string[] }> }) {
  ...
  return (
    <main className="shell">
      ...
      <DataScope stockDate={dashboard.stockDate} optionsDate={dashboard.optionsDate} expiration={dashboard.optionsExpiration} optionWindow={dashboard.optionWindowLabel} strikeCount={dashboard.optionOpenInterest.length} />
      ...
      <section className="section-block"><ModuleHeading index="01" kicker="PRICE STRUCTURE" title="价格趋势与关键位" description="价格处在什么趋势，离重要期权价位还有多远。" canAnswer="趋势位置与关键价位距离" cannotAnswer="突破后的必然涨跌方向" accent="#57d68d" />
        <SectionPager label="价格趋势与关键位视图" accent="#57d68d" tabs={[
          { id: "price-chart", label: "K线图", content: <div className="chart-panel"><div className="chart-gesture-note"><span>↔ 左右拖动查看历史</span><div className="compact-chart-legend"><i className="candle" />K<i className="ma20" />MA20<i className="ma50" />MA50<i className="ma200" />MA200</div></div><PriceChart data={dashboard.priceHistory} levels={{ maxPain: dashboard.options.maxPain, callWall: dashboard.options.callWall, putWall: dashboard.options.putWall, expectedUpper: dashboard.options.expectedUpper, expectedLower: dashboard.options.expectedLower }} /></div> },
      ...
      <section className="section-block"><ModuleHeading index="02" kicker="MOMENTUM & VOLATILITY" title="动量与波动率" description="判断走势是否过热，并比较近期实际波动与期权隐含定价。" canAnswer="动量冷热与波动定价差异" cannotAnswer="下一交易日涨跌或期权绝对贵贱" accent="#4f8cff" />
        <SectionPager label="动量与波动率视图" accent="#4f8cff" tabs={[
          { id: "momentum-overview", label: "动量与波动", content: <><MomentumVisual rsi={dashboard.trend.rsi14} realizedVolatility={dashboard.trend.rv20} /><MomentumInformation rsi={dashboard.trend.rsi14} realizedVolatility={dashboard.trend.rv20} atmIv={dashboard.options.atmIv} /></> },
      ...
      <section className="section-block"><ModuleHeading index="03" kicker="OPTIONS POSITIONING" title="期权持仓结构" description="观察未平仓量集中位置、预期区间与 Gamma 结构代理。" canAnswer="OI集中位置与到期波动定价" cannotAnswer="真实做市商方向或未来价格" accent="#f0b45c" />
        <OptionWindowSelector symbol={symbol} selected={dashboard.optionWindow} />
